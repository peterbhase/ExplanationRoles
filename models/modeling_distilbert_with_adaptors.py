...
class LayerAdaptor(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dropout = nn.Dropout(p=config.dropout)
        self.lin1 = nn.Linear(in_features=config.dim, out_features=config.d_adaptor)
        self.lin2 = nn.Linear(in_features=config.d_adaptor, out_features=config.dim)
        assert config.activation in ["relu", "gelu"], "activation ({}) must be in ['relu', 'gelu']".format(
            config.activation
        )
        self.activation = gelu if config.activation == "gelu" else nn.ReLU()
...
class TransformerBlock(nn.Module):
    def __init__(self, config):
        ...
        if self.use_adaptors:
            self.adaptor_layer1 = LayerAdaptor(config)
            self.adaptor_layer2 = LayerAdaptor(config)
    ...
    def forward(self, x, attn_mask=None, head_mask=None, output_attentions=False):
        ...
        if self.use_adaptors:
            sa_output = self.adaptor_layer1(sa_output)
        ...
        # Adaptor Network
        if self.use_adaptors:
            ffn_output = self.adaptor_layer2(ffn_output)
...
class DistilBertForSequenceClassification(DistilBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        assert hasattr(config, 'use_adaptors'), "please add use_adaptors to configu"
        self.use_adaptors = config.use_adaptors
    ...
    def train(self, *args, **kwargs):
        ...
        if self.use_adaptors:
            for n, p in self.named_parameters():
                # print(n)
                if 'adaptor_layer' in n or 'layer_norm' in n or 'LayerNorm' in n or 'pre_classifier' in n or 'classifier' in n:
